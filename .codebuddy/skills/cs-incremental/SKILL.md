...
argument-hint: "[task to implement incrementally]"
user-invocable: true
...
See `.codebuddy/references/cs-definition-of-done.md` for the project-wide bar.

Before each slice, follow [the task-context protocol](../../references/sysdocs-design-context.md): record semantic/documentation impact, combine summary and knowledge-base candidates with source checks, and expand flow reading for high-risk boundaries. Update affected descriptions, summaries, file indexes, incoming links and applicable requirements/decisions before fixing the review snapshot. Necessary reversible synchronization is included in the authorized task; use `cs-sysdocs-update` for existing-library maintenance without asking again or refreshing the whole library. No-documentation-impact needs a concrete reason. Missing SysDocs does not force initialization; explicitly requested complete documentation remains required. Verify structure and content separately, reuse unchanged evidence, and recheck conclusions after related code/document changes. Code and documents ship together; current-change omissions block completion, unrelated historical defects do not.
