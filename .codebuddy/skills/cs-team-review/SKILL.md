...
argument-hint: "<path | local git ref/diff | run-id>"
user-invocable: true
...
When project instructions or the assignment require documentation-driven development, the host supplies the advisor's Documentation-driven review inputs: project root, three-state SysDocs inventory evidence, permitted snapshots of `SysDocs/SYSTEM_ROOT.md` and manifest-selected modules/pages, relevant spec/ADRs, and available update/validator reports. Apply the inventory rules in `../../references/sysdocs-system.md`; absence of the directory does not opt a DDD project out. Record applicability, document identities and coverage in `scope.md`, and preserve the bounded snapshot/manifest rules for this context. Do not add schema v1 fields.

The run remains read-only: missing/stale context is returned to the host for maintenance outside the run, followed by a new linked run when the context changes. Bounded findings may still be collected, but unavailable evidence required for the assigned review blocks approval. Design/plan review checks module traceability and usable context; it does not demand reports from a future implementation. For delivery or documentation-impacting repair verification, require the corresponding document updates and snapshot-matched update/validator evidence. Record violations as ordinary located findings and missing prerequisite coverage in authorized Markdown. The host checks these narrative requirements separately; the Team Review JSON validator does not validate SysDocs or prove DDD compliance.
