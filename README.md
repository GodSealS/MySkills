...
> and the top-level `skills/`, `agents/`, `references/`, `commands/`)
> is generated by `scripts/build-adapters.ps1`. Root context files are maintained separately;
> `CLAUDE.md` is generated only when missing.
...
普通变更按“明确目标 → 必要时拆任务 → 实现 → 按影响验证”执行。已有需求、架构决策、
批准和通过的验证可复用；输入变化时重新核验受影响部分。

- Spec 和 Planning 仅在需求缺失或任务需要拆分时启用，PLAN/TASKS 共用一个任务列表。
- Grill Review 用于显式压力测试或重大设计的不确定假设，不是普通计划的前置门槛。
- 普通切片由当前代理完成；架构变化才咨询架构师，团队工作流仍须显式调用。
- 角色按需加载 0–3 个技能，没有最低配额；启动时不预加载技能正文。
- 每个切片运行受影响检查，在集成/最终交付时运行项目要求的完整检查，复用有效结果。
- `/cs-build` 在整份计划结束且已有知识库、索引源码发生变化时刷新一次；无库时不启动管理员。
...
Choose one active copy per skill in a workspace. If both project and user copies are
installed, disable the duplicate by its absolute path in `~/.codex/config.toml`, keeping
the files for distribution. For example, when the user copy is authoritative locally:

```toml
[[skills.config]]
path = "/absolute/project/.agents/skills/cs-planning/SKILL.md"
enabled = false
```

Repeat only for actual duplicate paths and restart Codex. Keep user skills synchronized
with the installer after rebuilding adapters. Global `AGENTS.md` contains only a fallback
router; project instructions are not copied into the global context.
See [Codex skill configuration](https://learn.chatgpt.com/docs/build-skills#enable-or-disable-local-codex-skills).
