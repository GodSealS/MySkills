...
1. **Preflight** — resolve the design doc, confirm a clean `git status`, create `<run-dir>/reviews/`, and establish the skill's three-state SysDocs baseline; pass SYSTEM_ROOT and affected module snapshots to the architect and advisor.
...
5. **Synthesize** — host obtains final `cs-sysdocs-update` + validator evidence, then FAN-OUT to `cs-review-advisor`: draft `<run-dir>/final-report.md` from tests, open findings and document verification; relevant experts verify recommendations and the host computes `SHIP|FIX FIRST`. Missing/failed DDD evidence prevents SHIP. Consult `cs-architect` only for architecture consequences.
...
In every execution round, the host arranges SysDocs synchronization and validation before fixing the review snapshot. Documentation-impacting repairs require verified document updates; later target changes invalidate affected reviews. Follow the skill's DDD evidence gate before DONE or committing.
